...
Clojure developers have often used clojure.core's pr and other similar
printing functions for writing Clojure data, and read or read-string
to read it.  For example, Leiningen's project.clj files are simply
Clojure lists that can contain elements that are symbols, keywords,
strings, vectors, and maps.

A subset of the data that can be written and read in this way is a
format called edn, Extensible Data Notation.
...
Motivation: In Clojure 1.4 and earlier, using clojure.core's read or
read-string to read such data from untrusted sources could leave your
application open to attack.
...
You can define pr-edn the same way, except using pr instead of pr-str
in its definition, and it will print the objects to *out*.
...
default-data-readers and *data-readers*).  Ensure that all tags you
wish to support in the data you read have a read handler defined for
them, and that you trust the behavior of those read handler functions.

The default-data-readers supplied with Clojure should be safe (i.e. it
is considered a serious bug if they are not).  The safety of any other
data reader functions you use is up to you.

TBD: Describe what library authors should do to make their code safe,
if they don't need the capabilities of clojure.core's read* functions.
Give examples of converting multiple-argument clojure.core/read calls
to clojure.edn/read calls with the correct options.
...
functions that do not explicitly bind *read-eval* to true or false.
Even with this, searching through your source code for all calls to
clojure.core's read* functions is strongly recommended.
...
(taken from https://github.com/edn-format/edn)
...
* symbols
* keywords
* integers.  Note that since *print-dup* is nil when printing above, if you print out and read back java.lang.Integer, Short, Byte, etc. types and read them back, they will become type java.lang.Long or clojure.lang.BigInt.
* floating point numbers.  Similarly as for integers, values read by clojure.edn/read* will come back as java.lang.Double's even if they were originally Float's.
...
previous section, but cannot be read with clojure.edn/read*, at least
not with Clojure 1.5.0-RC15:
...
* Clojure types defined with deftype, unless you define a print method
  and a data reader for them.  TBD: Give an example of how to do so.
* TBD: What else?


# Vulnerabilities in clojure.core's read and read-string

Clojure's clojure.core/read and read-string were designed for reading
Clojure code and data from trusted sources.

Using them to read data from untrusted sources when *read-eval* is
bound to true (its default value in all Clojure versions) makes it
trivial for an attacker to execute arbitrary code on your system:

    user=> (read-string "#=(clojure.java.shell/sh \"ls\")")
    {:exit 0, :out "antsetup.sh\nbuild.xml\nchanges.md\nclojure-1.5.0-master-SNAPSHOT.jar\nclojure.iml\nclojure.jar\ndoc\nepl-v10.html\nmaven-classpath\nmaven-classpath.properties\npom.xml\nqueue.patch\nreadme.txt\nsrc\ntarget\ntest\n", :err ""}
